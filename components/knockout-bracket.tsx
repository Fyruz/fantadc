...
type MatchRow = PublicKnockoutRound["matches"][number];
type MatchTeam = MatchRow["homeTeam"];

function TeamFlag({ team }: { team: MatchTeam }) {
  if (!team) return <div className="w-5 h-3.25 shrink-0" />;
  const src = resolveTeamFlag(team);
  if (!src) return <div className="w-5 h-3.25 shrink-0" />;
  return <img src={src} alt={team.name} width={20} height={13} className="shrink-0 object-cover" />;
}
...
function MatchCard({ m, cardRef }: { m: MatchRow; cardRef?: React.Ref<HTMLDivElement> }) {
  ...
  const row = (team: MatchTeam, seed: string | null, score: number | null, won: boolean, isLast: boolean) => (
    <div
    ...
    >
      ...
      <span
        ...
        style={{ color: team ? "var(--text-primary)" : "rgba(0,0,0,0.3)", fontWeight: won ? 600 : 400 }}
      >
        {team?.shortName ?? team?.name ?? seed ?? "TBD"}
      </span>
      {scored && (
        <span className="text-xs tabular-nums shrink-0" style={{ color: "var(--text-primary)", fontWeight: won ? 600 : 400 }}>
          {score}
        </span>
      )}
    </div>
  ...
  const card = (
    ...
    >
      {row(m.homeTeam, m.homeSeed, m.homeScore, homeWon, false)}
      {row(m.awayTeam, m.awaySeed, m.awayScore, awayWon, true)}
    </div>
  );

  return m.homeTeamId && m.awayTeamId ? (
    <Link href={`/partite/${m.id}`} className="block h-full transition-opacity hover:opacity-80">
      {card}
    </Link>
  ) : (
    card
...
export default function KnockoutBracket({ rounds }: { rounds: PublicKnockoutRound[] }) {
  const probeRef = useRef<HTMLDivElement>(null);
  const [measuredHeight, setMeasuredHeight] = useState<number | null>(null);

  const mainRounds = rounds.filter((r) => r.order !== 3 && r.matches.length > 0);

  useLayoutEffect(() => {
    if (probeRef.current) setMeasuredHeight(probeRef.current.offsetHeight);
  }, [mainRounds[0]?.matches[0]?.id]);

  if (rounds.length === 0) {
    return <div className="card p-10 text-center over-label">Fase ad eliminazione non ancora iniziata.</div>;
  }

  const CARD_HEIGHT = measuredHeight ?? FALLBACK_CARD_HEIGHT;
  const round0Count = mainRounds[0]?.matches.length ?? 0;
  const height = round0Count * CARD_HEIGHT + Math.max(0, round0Count - 1) * (CARD_HEIGHT / 2);
  const width = mainRounds.length * CARD_WIDTH + Math.max(0, mainRounds.length - 1) * COL_GAP;
    ...
    <div className="flex flex-col gap-6">
      {mainRounds[0] && (
        <div style={{ position: "fixed", top: -9999, left: -9999, width: CARD_WIDTH, visibility: "hidden" }} aria-hidden="true">
          <MatchCard m={mainRounds[0].matches[0]} cardRef={probeRef} />
        </div>
      )}
      {mainRounds.length > 0 && (
          ...
          <div style={{ minWidth: width }}>
            ...
            <div className="relative" style={{ width, height }}>
              <svg className="absolute inset-0 pointer-events-none" width={width} height={height} style={{ overflow: "visible" }}>
                {mainRounds.slice(0, -1).map((round, r) => {
                  const matchCount = round.matches.length;
                  const nextMatchCount = mainRounds[r + 1].matches.length;
