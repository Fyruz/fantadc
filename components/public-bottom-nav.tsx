...
// CSS filter che converte black → var(--primary) #0107A3
const FILTER_PRIMARY = "invert(8%) sepia(100%) saturate(7496%) hue-rotate(233deg) brightness(92%) contrast(101%)";
...
export default function PublicBottomNav() {
  ...
  return (
    <>
      ...
      {moreOpen && (
        <div className="fixed bottom-16 left-0 right-0 z-50 rounded-t-2xl md:hidden" style={{ background: "#fff", borderTop: "1px solid var(--border-soft)", boxShadow: "0 -4px 24px rgba(1,7,163,0.12)" }}>
          <div className="flex items-center justify-between px-4 pt-3 pb-1">
            <span className="text-xs font-black uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>ALTRO</span>
            <Button icon="pi pi-times" text type="button" onClick={() => setMoreOpen(false)} className="p-1!" style={{ color: "var(--text-muted)" }} aria-label="Chiudi" />
          </div>
          <div className="flex flex-col gap-1 px-3 pb-4">
            {DCUP_MORE.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setMoreOpen(false)}
                  className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-colors"
                  style={active ? { background: "var(--surface-1)", color: PRIMARY } : { color: "var(--text-secondary)" }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden" style={NAV_STYLE}>
        <div className="flex px-1 py-2.5">
          ...
          <Button unstyled type="button" onClick={() => setMoreOpen((v) => !v)} className="flex flex-1 flex-col items-center justify-center gap-1.5 px-2 py-1">
            <NavIcon src="/icons/more.svg" active={moreIsActive || moreOpen} />
            <span className="text-[10px]" style={navItemStyle(moreIsActive || moreOpen)}>Altro</span>
          </Button>
        </div>
      </nav>
    </>
